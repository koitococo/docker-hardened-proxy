...
func classifyTopLevel(parts []string, info RouteInfo) RouteInfo {
	// Single-segment paths: /_ping, /version, /info, /build, /grpc, /auth
	if len(parts) == 1 {
	...
	if parts[0] == "images" {
		...
		if len(parts) >= 3 {
			...
			// /images/{name}/json (inspect), /images/{name}/tag (name can contain slashes)
			if parts[len(parts)-1] == "json" || parts[len(parts)-1] == "tag" {
				return info
			}
		}
		info.Kind = Denied
